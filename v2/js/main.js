...
function uploadPhoto(obj){
	...
	if(typeof(obj) != "undefined" || obj != null){
		...
		if(fileName != "" && image != 'jpg' && image != 'jpeg'){
			...
			$('#msg-error-img').html("Extension no valida");
...
function uploadPhotoToServer(){
	...
	$.ajax({ type: "POST",  contentType: false, processData: false,  url: "../_source_ajax/files/upload_image_file.php", data: formData, cache: false, async: true,
		...
		success : function(response){
				...
				switch(response){
					case 'Error':
						$('#msg-error-img').html("Error exception");
					break;
					case 'noimage':
						$('#msg-error-img').html("Error, el archivo no es una imagen.");
					break;
					case 'errorsize':
						$('#msg-error-img').html("Error, el tamaño máximo permitido es un 1MB.");
					break;
					case 'success':
						$('#msg-error-img').html("La foto ha sido cargada correctamente.");
						$('#containerWindow').remove();
					    closeFrontPage()
					    loadScrips('../_source_show/files/home_page.php');	
					break;
					case 'errorupload':
						$('#msg-error-img').html("Ha ocurrido un error durante la carga de la foto intentelo de nuevo.");
					break;
				}
